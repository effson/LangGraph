...
要从LangGraph节点或工具内部发送自定义用户定义数据，遵循以下步骤：
-1. 使用get_stream_writer访问流写入器并发送自定义数据。
-2. 调用.stream()或.astream()时，设置stream_mode="custom"以在流中获取自定义数据。
可以组合多种模式（例如["updates", "custom"]），但至少有一种模式必须是"custom"。
...
def node(state: State):
    ...
    writer({"custom_key": "这是自定义输入的内容，O(∩_∩)O"})
